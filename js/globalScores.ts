...
/**
 * Trae el ranking global agregado por cantidad de partidas jugadas
 * (vista global_activity_rank, ver supabase/schema.sql), usado en el
 * panel "TOP GLOBAL" del HUD lateral. A diferencia de fetchGlobalTop,
 * no es por-juego: agrega todas las partidas de todos los minijuegos
 * por usuario, porque el HUD muestra un único ranking, no uno por
 * módulo.
 */
export async function fetchGlobalActivityRank(limit = 10): Promise<GlobalActivityRow[]> {
  try {
    const supabase = await getSupabaseClient();
    const { data, error } = await supabase
      .from('global_activity_rank')
      .select('username, games_played')
      .order('games_played', { ascending: false })
      .limit(limit);

    if (error || !data) {
      ErrorLogger.log('globalScores.fetchGlobalActivityRank', error, {});
      return [];
    }
