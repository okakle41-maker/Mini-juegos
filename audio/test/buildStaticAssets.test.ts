...
describe('vite build copia css/, assets/ y audio/ a dist/ (regresión: activos estáticos referenciados por ruta plana)', () => {
  beforeAll(() => {
  ...
  it('copia todos los .mp3 de audio/ a dist/audio/ (musicPlayer.ts los referencia por ruta plana)', () => {
    const srcAudioDir = resolve(__dirname, '../audio');
    const distAudioDir = resolve(DIST, 'audio');

    expect(existsSync(distAudioDir)).toBe(true);

    const srcFiles = readdirSync(srcAudioDir).filter((f) => f.endsWith('.mp3'));
    const distFiles = readdirSync(distAudioDir).filter((f) => f.endsWith('.mp3'));
