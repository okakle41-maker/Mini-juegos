...
function createChainableMock(): any {
  const chainable: any = {
    ...
    maybeSingle: vi.fn(async () => ({ data: { status: 'playing' }, error: null })),
...
describe('fix: leaveCurrentMatch sí marca abandoned cuando corresponde', () => {
  it('marca abandoned si la DB todavía tiene status playing', async () => {
    ...
    (lobbySystem as any).currentMatch = {
      id: 'match2', lobbyId: 'lobby1', gameId: 'arrow', status: 'playing',
