...
- Sistema completo en `multiplayerSystem.ts` usando Supabase Realtime, sobre las tablas `live_matches`/`match_messages` (`migration_005_coop_rooms.sql`/`migration_010_room_settings.sql`).
...
4. Corré en orden las migraciones restantes — cada una depende de que la anterior ya se haya aplicado:
   ...
   - **`migration_008_lobbies.sql`** — crea `lobbies`/`lobby_players`/`lobby_matches`/`lobby_match_messages`, el sistema de lobbies grupales (hasta 8 jugadores, con sub-partidas 1v1 de Simon/Arrow/Termita) que usa `lobbySystem.ts`. Paralelo a `live_matches` (migration_005), no lo reemplaza — Letters Fall sigue usando `live_matches` tal cual.
   - **`migration_009_lobby_expiration.sql`** — agrega `lobbies.last_activity_at` y una función de purga (`purge_stale_lobbies`, invocada oportunísticamente desde `createLobby`/`joinLobby`) que cierra lobbies sin actividad hace más de 30 minutos. Sin esto, un lobby abandonado sin salir explícitamente (pestaña cerrada) bloquea su código para siempre. **Requiere que `migration_008_lobbies.sql` ya se haya ejecutado.**
   - **`migration_010_room_settings.sql`** — agrega la columna de `settings` compartidos en `live_matches`, para que la dificultad que fija quien crea una sala (simon/arrow/termita/letters) se propague al segundo jugador. Independiente de las dos anteriores (toca `live_matches`, no `lobbies`) — el orden entre esta y las de lobbies no importa, alcanza con que las tres corran después de la 007.
   - **`migration_011_prevent_double_match.sql`** — trigger que rechaza que un jugador quede activo en dos sub-partidas de `lobby_matches` a la vez (antes, la sub-partida vieja quedaba huérfana en `'playing'` para siempre si el cliente perdía la referencia). **Requiere `migration_008_lobbies.sql`.**
   - **`migration_012_tournament_participant_identity.sql`** — corrige `tournament_participants` para usar el `auth.uid()` real en vez del string literal `'current_player'` para todos los usuarios; sin esto, cualquier inscripto podía borrar la inscripción de otro. **Requiere `migration_006_social_tournaments.sql`.**
   - **`migration_013_clan_leader_scope.sql`** — acota las políticas de `clans`/`clan_members` (que `migration_006` había dejado en `using(true)`) al liderazgo real de cada clan; sin esto, cualquier cliente con la anon key podía reescribir el clan de otro.
   - **`migration_014_friends_update_check.sql`** — agrega `WITH CHECK` a los `UPDATE` de `friends`/`friend_requests` (antes solo tenían `USING`, que en Postgres RLS no valida los valores *resultantes* del update); sin esto, quien recibe una solicitud pendiente podía reescribir `sender_id`/`receiver_id` hacia terceros arbitrarios.
   - **`migration_015_lobby_capacity_race_fix.sql`** — cierra una condición de carrera en el trigger de capacidad de `lobbies` (dos `INSERT` concurrentes en `lobby_players` podían ambos leer el mismo conteo antes de confirmar y dejar un lobby con 9/8 jugadores).
   - **`migration_016_signal_triangulation.sql`** (658 líneas) — crea `signal_triangulation_matches`/`_rounds`/`_locks`, el backend completo del minijuego cooperativo de 4 jugadores Signal Triangulation. No reusa `lobby_matches` (pensada para 2 jugadores fijos) — tablas nuevas paralelas. **Sin esta migración ese juego no funciona en absoluto.**
   - **`migration_017_ship_control.sql`** (1,334 líneas, la migración más grande) — crea las 7 tablas del minijuego cooperativo Centro de Control de una Nave (`ship_control_matches`/`_state`/`_actions`/`_events`/`_event_types`/`_event_progress`/`_morse_table`): máquina de estados con log de acciones para 4 jugadores simultáneos. **Sin esta migración ese juego no funciona en absoluto.**
   - **`migration_018_fragmented_labyrinth.sql`** — crea `fragmented_labyrinth_matches`/`_state`, backend del minijuego cooperativo Fragmented Labyrinth (portado de un prototipo standalone con servidor Node propio a RPC + RLS sobre Postgres, como los dos anteriores). **Sin esta migración ese juego no funciona en absoluto.**
   - **`migration_019_signal_triangulation_view_invoker_fix.sql`** — corrige el warning "Security Definer View" del linter de Supabase sobre dos vistas creadas en `migration_016`. **Requiere `migration_016_signal_triangulation.sql`.**
   - **`migration_020_generic_double_match_guard.sql`** — reemplaza 4 implementaciones casi idénticas del guard "un jugador no puede estar en dos partidas activas a la vez" (una por minijuego cooperativo, cada una a mano sobre columnas nombradas distinto) por una función genérica reutilizable. **Requiere que `migration_011`, `016`, `017` y `018` ya estén aplicadas** (toca triggers que esas migraciones crearon).
   - **`migration_021_generic_role_helpers.sql`** — mismo criterio que la 020, para los helpers "¿qué rol ocupo en esta partida?" (antes duplicados entre `ship_control` y `fragmented_labyrinth`). **Requiere `migration_017_ship_control.sql` y `migration_018_fragmented_labyrinth.sql`.**
   - **`migration_022_ship_control_morse_rls.sql`** — habilita RLS en `ship_control_morse_table`, la única tabla nueva desde `schema.sql` a la que `migration_017` no se lo había activado (auditoría posterior detectó el gap). Sin ella, esa tabla queda con lectura/escritura abierta a cualquier cliente con la anon key. **Requiere `migration_017_ship_control.sql`.**
5. Verificá en **Table Editor** que aparecieron las tablas `profiles`, `scores`, `live_matches`, `match_messages`, `friends`, `clans`, `lobbies`, `lobby_players`, `lobby_matches`, `signal_triangulation_matches`, `ship_control_matches`, `fragmented_labyrinth_matches`, etc., y la vista `best_scores`.
...
Sin el paso 2, el registro/login va a fallar (la tabla `profiles` no existe). Sin el paso 6, el registro parece fallar con un error de "no se pudo iniciar sesión automáticamente" pese a que las tablas estén bien. El paso 3 no bloquea el uso básico (todo funciona sin él), pero sin rate limiting el scoreboard global queda expuesto a manipulación. El paso 4 es necesario para que multiplayer, social, torneos y los 3 minijuegos cooperativos de 4 jugadores (Signal Triangulation, Ship Control, Fragmented Labyrinth) funcionen contra datos reales — sin él, esas pantallas cargan pero cualquier operación de escritura falla contra tablas inexistentes.

> **No hay `schema.sql` consolidado con las 22 migraciones ya aplicadas** (Aug 2026): el proyecto no usa el CLI formal de Supabase (`supabase/config.toml`/`supabase/migrations/` no existen — estos archivos son SQL nombrado a mano, pensado para pegarse uno por uno en el SQL Editor del dashboard), así que no hay forma de generar ese consolidado sin conectarlo a un Postgres real (`supabase db dump`, que requiere Docker o un proyecto remoto — ninguno disponible al escribir esto). Fusionar las ~4,700 líneas a mano no es seguro: varias migraciones hacen `DROP POLICY`/`ALTER` sobre objetos que una migración anterior creó (`tournament_participants` sola se toca en 18 migraciones distintas), así que un merge por texto sin poder ejecutarlo y validarlo contra una base real arriesga quedar sutilmente incorrecto. Si en algún momento hay acceso a Docker o a un proyecto Supabase de staging, `supabase init` + copiar estos 23 archivos a `supabase/migrations/<timestamp>_<nombre>.sql` (en este mismo orden) + `supabase db diff -f consolidated` sobre una base con todas aplicadas es el camino correcto para generar ese archivo de forma confiable.
...
| **`security.test.ts`** | Suite completa de `js/security.ts`: `escapeHtml`, `randomInt` (incluyendo la rama de rechazo por muestreo no uniforme, forzada mockeando `crypto.getRandomValues`), `sanitizeInput` (truncado, remoción de control chars, combinaciones de `allowHtml`/`allowScript`, limpieza iterativa anti-bypass), `isSafeString` (los 12 patrones peligrosos reconocidos), `safeJsonParse`, y `RateLimiter`/`globalRateLimiter` con fake timers (ventanas de tiempo, `reset`/`clear`, identificadores independientes). Documenta (sin corregir) un bug real: `safeJsonParse` usa `key in parsed` para chequear claves peligrosas (`__proto__`, `constructor`, `prototype`), pero ese operador recorre la cadena de prototipos — como todo objeto/array hereda esas claves de `Object.prototype`, la condición es `true` siempre, y la función rechaza el 100% de los objetos/arrays JSON legítimos, no solo los maliciosos. Confirmado que no tiene ningún caller en código de producción, así que hoy no rompe nada, pero quedaría inutilizable si alguien la adoptara sin arreglarla primero. |
| **`copyRoomCode.test.ts`** | `attachCopyButton` (`js/utils/copyRoomCode.ts`): inserción/reutilización del botón sin duplicarlo, reemplazo del listener anterior por clonado de nodo al re-invocar sobre el mismo `buttonId`, copia del `textContent` (con trim) al portapapeles, ciclo éxito → notificación → reset del label con fake timers (incluyendo que un segundo click reinicia el temporizador), camino de fallo de `navigator.clipboard.writeText`, y el fallback a `document.execCommand('copy')` cuando la Clipboard API no existe (éxito y el caso en que `execCommand` devuelve `false`). |
| **`errorLogger.test.ts`** | `ErrorLogger` (`js/core/errorLogger.ts`): captura de `Error` (message/stack) vs. valores no-Error normalizados con `String()`, timestamp ISO, `meta` por defecto vs. explícito, buffer circular en el límite de 50 entradas, que `recent()` devuelve una copia (mutarla no afecta el estado interno), `setSink`/reemplazo de sink, y que una excepción lanzada por el sink se captura y reporta sin interrumpir el registro del error original. |
| **`gameHelpers.test.ts`** | `GameHelpers` (`js/utils/gameHelpers.ts`): `clamp`, `readClampedInt` (sobre `<input>`/`<select>`, valores fuera de rango o no numéricos), `shakeElement` con fake timers (verificando que la clase se remueve antes de re-agregarse, para forzar el reflow), `updateProgressBar` (redondeo, división por cero, clamping 0–100), `shuffle` (no muta el array original, mismo multiset, y un caso analítico con `Math.random` mockeado a `0` que verifica el resultado exacto del Fisher-Yates), y `createCleanupManager` (listeners vía `AbortSignal`, intervals/timeouts cancelados por `cleanup()`, reusabilidad tras `cleanup()`, managers independientes que no interfieren entre sí). |
| **`safeStorage.test.ts`** | `SafeStorage` (`js/core/safeStorage.ts`) en sus dos escenarios de arranque: disponible y no disponible (forzando que la sonda del constructor lance, como en modo privado de Safari). Cubre los tres pares `get`/`set` (JSON, string, number), el flujo de `options.validate` con auto-borrado de la clave corrupta, manejo de excepciones en cada método, casos numéricos límite (`NaN`, `Infinity`, `"0"` vs. clave inexistente), y round-trips reales contra un `Map` que simula el storage. |
| **`debounceThrottle.test.ts`** | Las tres utilidades de `js/utils/debounceThrottle.ts` con fake timers: `debounce` (reinicio del temporizador con cada llamada, solo los argumentos de la última llamada sobreviven), `throttle` (ejecución inmediata en la primera llamada, llamadas ignoradas durante la ventana que **no** quedan encoladas — a diferencia de debounce), y `rafThrottle` (coalescing de múltiples llamadas en un único frame, con `requestAnimationFrame`/`cancelAnimationFrame` mockeados a mano ya que jsdom no los dispara solo). |
| **`describeMatchError.test.ts`** | `describeMatchError` (`js/utils/describeMatchError.ts`): las tres ramas documentadas en el propio archivo — sin conexión (`navigator.onLine` falso), los 6 patrones de error de red crudo del navegador (case-insensitive), paso directo del mensaje cuando ya es un `Error` de aplicación con mensaje propio, y el fallback genérico con sufijo de reintento para errores no reconocibles. Incluye un test explícito de precedencia: un error que matchea patrón de red gana aunque también sea un `Error` con mensaje propio. |
| **`buttonBusyGuard.test.ts`** | `withButtonBusy` (`js/utils/buttonBusyGuard.ts`): `disabled`/`aria-busy` mientras la acción está en vuelo (con una promesa controlada a mano para inspeccionar el estado intermedio), limpieza vía `finally` tanto en éxito como en rechazo (propagando el error, no silenciándolo), los casos `btn` `null`/`undefined`, y el camino reentrante — si el botón ya está `disabled` por una llamada anterior en vuelo, corre la acción directo sin volver a tocar `aria-busy`. |
| **`asyncEventHandler.test.ts`** | `onClickAsync`/`onClickAsyncVoid` (`js/utils/asyncEventHandler.ts`): que el wrapper devuelto es síncrono, que invoca el handler original con o sin el `Event` según corresponda, que un rechazo del handler se captura y loguea vía `console.error` en vez de quedar como unhandled rejection, y que un fallo en una invocación no contamina invocaciones futuras del mismo wrapper. |
| **`backButton.test.ts`** | `hydrateBackButtons` (`js/utils/backButton.ts`): botones sin `data-back-to` se ignoran, labels por defecto (`home`/`multiplayer`) vs. fallback genérico vs. `data-back-label` explícito, `aria-label` correcto, wiring del click a `window.backToMenu` con el target correcto, **idempotencia** (una segunda llamada no re-hidrata ni duplica el listener — verificado contando invocaciones reales, no solo el estado del DOM), múltiples botones en una sola pasada, scoping a un `root` distinto de `document`, y un botón ya marcado `data-hydrated` externamente. |
| **`categorySlug.test.ts`** | Refuerzo de edge cases para `categorySlug` (`js/utils/categorySlug.ts`) más allá de los 4 casos base ya cubiertos en `selfCompare.test.ts`: colapso de espacios/símbolos consecutivos en un único guion, trim de guiones en los bordes del resultado, preservación de dígitos, inputs vacíos o compuestos solo por símbolos, y el caso de la `Ñ` (se descompone bajo `normalize('NFD')` en `N` + tilde combinante, así que sí se preserva como `n` — no es un caso "que se pierde" como podría asumirse a primera vista). |
...
### Resuelto: `safeJsonParse` rechazaba cualquier objeto/array JSON legítimo

`js/security.ts` exporta `safeJsonParse<T>(json, fallback)`, pensada para parsear JSON de forma segura contra prototype pollution. Tras parsear, validaba que el resultado no tuviera las claves `__proto__`/`constructor`/`prototype` con `if (key in parsed)`. El problema: el operador `in` recorre la cadena de prototipos, no solo propiedades propias — y **todo** objeto o array de JS hereda `constructor` de `Object.prototype` (o `Array.prototype`), así que esa condición daba `true` para cualquier objeto/array no vacío, incluso uno perfectamente inocente como `{"nombre":"jugador1","puntaje":100}` o `[1,2,3]`. La función devolvía siempre el `fallback`, para cualquier input que no fuera un primitivo (string/número/booleano/`null`).

**Corregido:** se reemplazó `key in parsed` por `Object.prototype.hasOwnProperty.call(parsed, key)`, que chequea solo propiedades propias del objeto ya parseado, no las heredadas de `Object.prototype`/`Array.prototype`. Con el fix, `{"nombre":"jugador1","puntaje":100}` y `[1,2,3]` se parsean correctamente, mientras que un objeto con `__proto__`/`constructor`/`prototype` como propiedad *propia* (p. ej. `{"__proto__":{"polluted":true}}`, que `JSON.parse` sí crea como propiedad propia) sigue siendo rechazado — que es el caso real de prototype pollution que la función busca prevenir. `test/security.test.ts` se actualizó para verificar el comportamiento correcto en vez de fijar el bug. Al momento del fix seguía sin tener callers en código de producción, pero queda utilizable para quien la adopte.
