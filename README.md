...
│   ├── backgroundManager.ts      # Tema oscuro/pixel, fondo, scanlines
...
│   ├── lobby-*.css                 # Variantes de header/lobby (tactical, pixel, new)
...
| **`backgroundManager.ts`** | Tema activo (oscuro / pixel), fondo, scanlines | |
...
- **`css/lobby-*.css`**: variantes visuales del header/lobby (`tactical`, `pixel`, `new`, `header-redesign`) — los cuatro se enlazan simultáneamente en `index.html`; `lobby-pixel.css` solo tiene efecto cuando `body[data-theme="pixel"]` está activo (ver comentario en `index.html`), el resto aplica siempre.
